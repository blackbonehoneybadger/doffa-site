 ...
 *
 * Всё считается в BigInt, и вот почему — не потому, что наши числа огромны:
 * 100 млн токенов с 6 знаками это 10^14 базовых единиц, и они как раз
 * помещаются в double (MAX_SAFE_INTEGER ≈ 9·10^15).
 *
 * Причины настоящие:
 *   1. Суммы SPL-токенов — это u64, предел 1.8·10^19. Это на три порядка выше
 *      безопасного целого в double, и общий код обязан работать с величинами,
 *      которые в number просто не представимы.
 *   2. Разбор дробей через float даёт накопленную ошибку: 12.3 * 1e6 не всегда
 *      ровно 12300000. При сжигании это означало бы уничтожение не той суммы,
 *      причём необратимо.
 *   3. API @solana/spl-token принимает bigint. Конвертация туда-обратно через
 *      number — лишний шанс потерять точность на ровном месте.
 */
