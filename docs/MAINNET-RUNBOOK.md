...
Всю последовательность выпуска умеет прогнать один скрипт:

```bash
npm run token:go -- --plan   # показать план, ничего не выполнять
npm run token:go             # выполнить
```

Он делает ровно те же шаги, что расписаны ниже, останавливается на первой же
ошибке и пропускает то, что уже сделано, — обрыв на середине лечится
перезапуском. В mainnet он дополнительно откажется работать, если:

- нет следа завершённого devnet-прогона (`token/deploy-output/devnet.json`);
- не задан `DOFFA_METADATA_URI` или ссылка ведёт на временный хостинг;
- подписант не совпадает с владельцем;
- ввод перенаправлен (пайп, CI) — фразу подтверждения должен печатать человек;
- фраза `CREATE MAINNET DOFFA` введена неточно.

Чего он **не** делает: не отзывает mint/freeze authority, ничего не сжигает,
не трогает старый токен. Это отдельные команды с отдельными фразами.

Дальше — тот же порядок по шагам. Читать стоит в любом случае: команда
экономит набор текста, но не понимание того, что происходит.
