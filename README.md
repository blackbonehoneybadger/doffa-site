...
| `ADMIN_SESSION_SECRET` | Отдельный секрет для подписи админ-cookie (не тот же, что `SESSION_SECRET`) |
| `SESSION_SECRET` | Секрет пользовательских nonce / профиля |

Админ-сессии серверные (таблица `admin_sessions`, срок ~12 ч, отзыв при logout).
Вход в `/admin` ограничен по IP (после 5 неверных попыток — блок на 15 минут).
