...
Игровое направление **DOFFA Games** (главная публичная игра — **DOFFA Bean Duel**)
и её сервер живут в отдельном репозитории — здесь только сайт `doffa.coffee`.
Публично сайт показывает только DOFFA Games и Bean Duel.

Модель экономики: **Tap → зёрна → Bean Duel → победа → Claim → $DOFFA**.
...
- **DOFFA Games / DOFFA Bean Duel** — игровое направление и главная публичная
  игра-дуэль экосистемы (отдельный репозиторий).
