...
Игра **DOFFA Defense** и её сервер живут в отдельном репозитории
(`zapisnoy-kozel`) — здесь только сайт `doffa.coffee`.

Модель экономики: **Tap → зёрна → DOFFA Defense → прохождение глав → Claim → $DOFFA**.
...
- **DOFFA Defense** — action-игра экосистемы (отдельный репозиторий).
